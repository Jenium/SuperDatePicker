...
export const SuperDatePicker: FC = () => {
  ...
  const handleAction = (action: string) => {
    action === 'next' ? setCurrentMonth(currentMonth + 1) : setCurrentMonth(currentMonth - 1);
  };
  ...
  useEffect(() => {
    const hanleClickDocument = (e) => {
      e.target.querySelector('.SuperDatePicker') ? setIsVisible(false) : null;
    };

    document.addEventListener('click', hanleClickDocument);
    return () => {
      document.removeEventListener('click', hanleClickDocument);
    };
  }, []);
